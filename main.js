...
function updateCurrentList() {
    if (currentTab === 'favorite') {
        currentList = vocabulary.filter(v => 
            favorites.some(f => f.hanzi === v.hanzi)
        );
    } else {
...
function isFavorite(vocab) {
    return favorites.some(f => f.hanzi === vocab.hanzi);
}
...
function updateCard() {
    if (currentList.length === 0) {
        hanziEl.textContent = "Không có từ nào";
    ...
    hanziEl.textContent = vocab.hanzi || "";
    pinyinEl.textContent = vocab.pinyin || "";
    meaningEl.textContent = vocab.meaning || "";
    englishEl.textContent = vocab.english || "No English";
...
function toggleFavorite() {
    ...
    const index = favorites.findIndex(f => f.hanzi === currentVocab.hanzi);
